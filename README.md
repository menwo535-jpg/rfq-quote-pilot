...
For restart recovery, idempotent imports and preserved correction history, see the optional [persistent draft API](PERSISTENT-JOBS.md). It uses SQLite from the Python standard library. The current source and [persistent-jobs ZIP](rfq-pilot-persistent-jobs-demo.zip) include this feature; older named ZIP packages below preserve their earlier versions.

The [rfq-pilot-n8n-demo.zip](rfq-pilot-n8n-demo.zip) package preserves the earlier stateless API, PDF/TXT fixtures, catalogue, Python tests, n8n workflow and integration verifier; it predates persistent jobs. The previous `rfq-pilot-xlsx-demo.zip` contains the Python exporter but no n8n integration; the older `rfq-pilot-demo.zip` also predates native XLSX. Use the matching source and requirements from one package together. A generated [Excel sample](output/quote-draft.xlsx) is included.
...
The 66 Python tests cover matching, money rounding, malformed inputs, Excel/HTTP output and persistent drafts. These include the prior 47 checks and 19 new SQLite/HTTP checks for restart recovery, request replay, correction history, conflicting edits and transaction rollback. The suite uses the independent openpyxl reader. A separate [seven-case n8n execution check](n8n/validation.json) previously verified real `/quote` requests, review routing, unchanged workbook bytes and validation failures stopping the workflow; it does not exercise the new `/jobs` API. `make_sample_pdf.py` regenerates the synthetic fixture using reportlab; reportlab is not needed to run the delivered CLI or adapter. Native Microsoft Excel, n8n Cloud and customer deployment have not been validated.
