...
The exported sample is also imported and rendered with the bundled spreadsheet tool for visual review. Native Microsoft Excel and n8n execution are not claimed. The previous separately authored formula-based demonstration used Codex's spreadsheet runtime; it is distinct from this portable values-only exporter and is not required to run it.
